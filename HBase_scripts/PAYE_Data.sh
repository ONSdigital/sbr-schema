#!/bin/bash
link="Link_Data.txt"
table_headers=(payeref ubrn dec_jobs mar_jobs june_jobs sept_jobs jobs_lastupd legalstatus stc nameline tradstyle address1 address2 address3 address4 address5 postcode)
m_table_headers=(payeref ubrn dec_jobs mar_jobs june_jobs sept_jobs jobs_lastupd legalstatus stc nameline tradstyle address1 postcode)
    	...
    	#----------------------------------------
    		if [[ " ${table_headers[@]} " =~ " ${headers[i]} " ]]; then
			    ...
			    if [[ "${headers[i]}" == "ubrn" ]] ; then
			    	rn_64=$(echo -n "PAYE~$payeref_num" | base64)
					...
					header_64=$(echo -n "l:c_$payeref_num" | base64)
					val_64=$(echo -n "PAYE" | base64)
					data=$(echo '{"Row":[{"key":"'$rn_64'", "Cell": [{"column":"'$header_64'", "$":"'$val_64'"}]}]}')
					/bin/bash HBase_scripts/Call_API.sh "$data" $link_url &
			    fi
			fi
